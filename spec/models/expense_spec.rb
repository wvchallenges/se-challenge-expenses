...
describe Expense do
	...
	describe "unique import" do
		...
		it "creates a new customer" do
			expect(Category).to receive(:find_or_create_by).with({name: row[:category]}).and_call_original
		end

		it "creates a new employee" do
			expect(Employee).to receive(:find_or_create_by).with({name: row[:employee_name], address: row[:employee_address]}).and_call_original
		end
