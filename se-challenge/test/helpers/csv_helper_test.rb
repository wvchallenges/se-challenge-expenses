...
class CsvHelperTest < ActionDispatch::IntegrationTest
  test "DesignIntent" do
    csv_input = <<-EOS
...
2/18/2014,Travel,Eric Schmidt,"1600 Amphitheatre Parkway, Mountain View, CA 94043",Airplane ticket to NY," 1,500.00 ",CA Sales tax, 112.50
	EOS
	hash_output = CsvHelper.csv_to_hash(csv_input)
	assert_equal "12/1/2013", hash_output[0][:date]
	assert_equal "Travel", hash_output[0][:category]
	assert_equal "Microsoft Office", hash_output[3][:expense_description]
	assert_equal 67.43, hash_output[3][:tax_amount]
	assert_equal "Macbook Air", hash_output[10][:expense_description]
	assert_equal 1999, hash_output[10][:pretax_amount]
  end
end
