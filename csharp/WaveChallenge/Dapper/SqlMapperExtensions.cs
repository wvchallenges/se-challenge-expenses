...
namespace WaveChallenge.Dapper
{ //#pragma warning disable 1573, 1591 // xml comments
    ...
    public static class SqlMapperExtensions
    {
        ...
        /// <summary>
        ///     Returns a single entity by a single id from table "Ts" asynchronously using .NET 4.5 Task. T must be of interface
        ///     type.
        ///     Id must be marked with [Key] attribute.
        ///     Created entity is tracked/intercepted for changes and used by the Update() extension.
        /// </summary>
            ...
            IDbTransaction transaction = null, int? commandTimeout = null) where T : class
        {
            ...
            {
                obj =
                    (await connection.QueryAsync<T>(sql, dynParms, transaction, commandTimeout).ConfigureAwait(false))
                        .FirstOrDefault();
            }
            return obj;
        }
            ...
            int? commandTimeout = null) where T : class
        {
            ...
            IEnumerable<PropertyInfo> computedProperties = ComputedPropertiesCache(type);

            var keyAndCompoundProperties = computedProperties;
            if (keyProperties.Any() && keyProperties.First().DeclaringType == typeof (int))
            {
                keyAndCompoundProperties = keyAndCompoundProperties.Union(keyProperties);
            }

            IEnumerable<PropertyInfo> allPropertiesExceptKeyAndComputed =
                allProperties.Except(keyAndCompoundProperties);

            for (int i = 0; i < allPropertiesExceptKeyAndComputed.Count(); i++)
            {
                PropertyInfo property = allPropertiesExceptKeyAndComputed.ElementAt(i);
            ...
            IDbTransaction transaction = null, int? commandTimeout = null) where T : class
        {
            ...
            IEnumerable<PropertyInfo> computedProperties = ComputedPropertiesCache(type);
            IEnumerable<PropertyInfo> allPropertiesExceptKeyAndComputed =
                allProperties.Except(keyProperties.Union(computedProperties));
            ...
            int? commandTimeout = null) where T : class
        {
            ...
            IEnumerable<PropertyInfo> allProperties = TypePropertiesCache(type);
            IEnumerable<PropertyInfo> computedProperties = ComputedPropertiesCache(type);
            IEnumerable<PropertyInfo> nonIdProps = allProperties.Except(keyProperties.Union(computedProperties));
        ...
        /// <summary>
        ///     Updates entity in table "Ts" asynchronously using .NET 4.5 Task, checks if the entity is modified if the entity is
            ...
            IDbTransaction transaction = null, int? commandTimeout = null) where T : class
        {
            ...
            int updated =
                await
                    connection.ExecuteAsync(sb.ToString(), entityToUpdate, commandTimeout: commandTimeout,
            ...
            IDbTransaction transaction = null, int? commandTimeout = null) where T : class
        {
            ...
            int deleted =
                await
                    connection.ExecuteAsync(sb.ToString(), entityToDelete, transaction, commandTimeout)
            ...
            int? commandTimeout = null) where T : class
        {
            Type type = typeof (T);
            string name = GetTableName(type);
            string statement = String.Format("delete from {0}", name);
            ...
            IDbTransaction transaction = null, int? commandTimeout = null) where T : class
        {
            ...
            string statement = String.Format("delete from {0}", name);
            int deleted =
                await connection.ExecuteAsync(statement, null, transaction, commandTimeout).ConfigureAwait(false);
            return deleted > 0;
        }
...
// do not want to depend on data annotations that is not in client profile
    ...
    public class SqlServerAdapter : ISqlAdapter
    {
            ...
            object entityToInsert)
        {
            ...
            IEnumerable<dynamic> r =
                await
                    connection.QueryAsync<dynamic>("select @@IDENTITY id", transaction: transaction,
    ...
    public class PostgresAdapter : ISqlAdapter
    {
            ...
            object entityToInsert)
        {
            ...
            IEnumerable<dynamic> results =
                await
                    connection.QueryAsync<dynamic>(sb.ToString(), entityToInsert, transaction, commandTimeout)
                        .ConfigureAwait(false);
    ...
    public class SQLiteAdapter : ISqlAdapter
    {
            ...
            string columnList, string parameterList, IEnumerable<PropertyInfo> keyProperties, object entityToInsert)
        {
            ...
            var id = connection.Execute(cmd, entityToInsert, transaction, commandTimeout);
            
            if (keyProperties.Any() && keyProperties.First().DeclaringType == typeof(int))
            {
                IEnumerable<dynamic> r = connection.Query("select last_insert_rowid() id", transaction: transaction,
                commandTimeout: commandTimeout);
                id = (int)r.First().id;
                keyProperties.First().SetValue(entityToInsert, id, null);
            }
            ...
            object entityToInsert)
        {
            ...
            IEnumerable<dynamic> r =
                await
                    connection.QueryAsync<dynamic>("select last_insert_rowid() id", transaction: transaction,
                        commandTimeout: commandTimeout).ConfigureAwait(false);
            var id = (int) r.First().id;
            if (keyProperties.Any())
                keyProperties.First().SetValue(entityToInsert, id, null);
