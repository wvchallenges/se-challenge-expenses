...
class ExpenseCatagorySerializer(serializers.HyperlinkedModelSerializer):
    ...
    def create(self, validated_data):
        expense_catagory = ExpenseCatagory.objects.filter(expense_catagory=validated_data["catagory"])
        if len(expense_catagory):
            return expense_catagory[0]
        return ExpenseCatagory(expense_catagory=validated_data["catagory"])
