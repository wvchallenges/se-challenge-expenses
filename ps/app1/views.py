...
def home(request):
  if request.method == "POST" and request.FILES['file']:
    ...
    if path.isfile(settings.MEDIA_ROOT+'/'+File.name): remove(settings.MEDIA_ROOT+'/'+File.name)
...
def write_to_database(Lines):
  ...
  for line in Lines:
    ...
    expense = Expense(date=line[0], category=line[1], name=line[2], address=line[3], description=line[4], pretax_amount=line[5], tax_name=line[6], tax_amount=line[7]) 
    expense.save()
...
def read_from_database():
  ...
  for i in range(Months):
    ...
    sum = t.aggregate(sum = Sum(F('pretax_amount')+F('tax_amount'), output_field=FloatField()))['sum']
    L.append([y,m,sum])
    m = m + 1
    if m > 12:
      y = y + 1
